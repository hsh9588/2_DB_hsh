...
-- 0 : 숫자 한칸을 의미, 여러 개 작성 시 오른쪽 정렬 + 빈칸에 0 추가
...
SELECT TO_CHAR(1234, '00000') FROM DUAL; -- ' 1234'
...
SELECT SYSDATE FROM DUAL; --2022-09-01 11:20:40.000

-- 2022/09/01 11:20:40 목요일
...
-- R 패턴 : 1세기를 기준으로 절반(50년) 이상인 경우는 이전 세기(1900년대)
...
SELECT EMP_NAME, DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남성', '2', '여성') AS 성별
FROM EMPLOYEE;
...
-- 급여가 최소값, 가장 빠른 입사일, 이메일 알파벳 순서가 가장 빠른 이메일

SELECT MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL)
FROM EMPLOYEE;

-- 급여가 최대값, 가장 느린 입사일, 이메일 알파벳 순서가 가장 느린 이메일
