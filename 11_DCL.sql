...
* 사용자 계정 : 데이터베이스에 대하여 질의, 갱신, 보고서 작성 등의
                ...
                ex) kh_hsh계정(각자 이니셜 계정), updown, workbook 등
*/
...
CREATE USER hsh_sample IDENTIFIED BY sample1234;
...
GRANT CREATE SESSION TO hsh_sample;
...
GRANT CREATE TABLE TO hsh_sample;

ALTER USER hsh_sample DEFAULT TABLESPACE
...
GRANT CONNECT, RESOURCE TO hsh_sample;
...
-- kh_hsh / hsh_sample 사용자 계정끼리 서로 객체 접근 권한 부여

-- 1. (sample) kh_hsh 계정의 EMPLOYEE 테이블 조회
SELECT * FROM EMPLOYEE;
--ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
--> 접근 권한이 없어서 조회 불가

-- 2. (kh) hsh_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO hsh_sample;

-- 3. (sample) 다시 kh_hsh.EMPLOYEE 조회
SELECT * FROM kh_hsh.EMPLOYEE;
...
-- 4. (sample) 계정 kh_hsh.EMPLOYEE 테이블을 복사한 테이블 생성
CREATE TABLE EMP_SAMPLE
AS SELECT * FROM kh_hsh.EMPLOYEE;
...
REVOKE SELECT ON EMPLOYEE FROM hsh_sample;
