...
/*
SQL (Structured Query Language) Structured : 구조적, Query : 질의, Language : 언어
DQL -> SELECT 					용도 : 데이터 검색
DML -> INSERT, UPDATE, DELETE    용도 : 데이터 조작
DDL -> CREATE, DROP, ALTER 		용도 : 데이터 정의
DCL -> GRANT, REVOKE				용도 : 데이터 제어
TCL -> COMMIT, ROLLBACK			용도 : 트랙젝션 제어
...
LOB : CLOB -> 가변길이 문자 (최대 4기가 바이트)
 ...
 - 데이터 조회(SELECT)하면 조건에 맞는 행들이 조회됨.
...
/*
 컬럼명 AS 별칭 : 별칭 띄워쓰기 x, 특수문자 x, 문자만 o
 
 컬럼명 AS "별칭" : 별칭 띄워쓰기 o, 특수문자 o, 문자만 o
...
--> " " 쌍따옴표는 특수문자, 기호, 대소문자 등을 구분하여 나타낼 때 사용하는 표기법
-- 		쌍따옴표 안에 작성되는 것들이 하나의 임을 의미
...
-- EMPLOYEE 테이블에서 입사일이 1990-01-01 ~ 1999-12-31 사이의 직원의
-- 이름, 입사일 조회
...
SELECT EMP_NAME, EMAIL
FROM EMPLOYEE
WHERE EMAIL LIKE '___#_%' ESCAPE '#'; -- 4글자 이상
-- # 뒤에 작성된 _는 일반 문자로 탈출

-- 연습문제
-- EMPLOYEE 테이블에서 이메일 '_' 앞이 4글자 이면서
-- 부서코드가 'D9' 또는 'D6'이고 -> AND가 OR보다 우선순위가 높다
