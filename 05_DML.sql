...
VALUES('900', '장채현', '901230-2345678', 'jang_ch@or.kr', '01048785623', 'D1', 'J7', 'S3', 4300000, 0.2, 200, SYSDATE, NULL, 'N');
...
FROM EMPLOYEE2 E 
LEFT JOIN DEPARTMENT2 D ON (DEPT_CODE = DEPT_ID);
...
UPDATE DEPARTMENT2 SET
DEPT_ID = 'D0', DEPT_TITLE = '전략기획2팀' -- WHERE절 직전 컬럼 작성 후 콤마 x
WHERE DEPT_ID = 'D9'
...
-- 유재식 급여
SELECT SALARY FROM EMPLOYEE2 E WHERE EMP_NAME = '유재식';

-- 유재식 보너스
SELECT BONUS FROM EMPLOYEE2 E WHERE EMP_NAME = '유재식';

-- 박명수 급여, 보너스 수정

UPDATE EMPLOYEE2 SET
SALARY = (SELECT SALARY FROM EMPLOYEE2 E WHERE EMP_NAME = '유재식'),
BONUS = (SELECT BONUS FROM EMPLOYEE2 E WHERE EMP_NAME = '유재식')
WHERE EMP_NAME = '방명수';
...
DELETE FROM EMPLOYEE2 E; --> WHERE 절 미작성 시 전체 삭제
