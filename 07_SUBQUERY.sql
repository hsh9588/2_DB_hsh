...
LEFT JOIN DEPARTMENT D ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE
HAVING SUM(SALARY)= (SELECT MAX(SUM(SALARY)) FROM EMPLOYEE E GROUP BY DEPT_CODE);                 
...
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT D ON (DEPT_CODE = DEPT_ID)
WHERE EMP_ID IN (SELECT DISTINCT MANAGER_ID FROM EMPLOYEE E WHERE MANAGER_ID IS NOT NULL);
...
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT D ON (DEPT_CODE = DEPT_ID)
WHERE EMP_ID NOT IN (SELECT DISTINCT MANAGER_ID FROM EMPLOYEE E WHERE MANAGER_ID IS NOT NULL);
...
-- 선택함수 사용!
--> DECODE(컬럼명, 조건1, 값1, 조건2, 값2, .... 일치하지 않는 경우)
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME,
CASE WHEN EMP_ID IN (SELECT DISTINCT MANAGER_ID FROM EMPLOYEE E WHERE MANAGER_ID IS NOT NULL) 
	 THEN '사수' 
	 ELSE '사원'
END
FROM EMPLOYEE E
...
FROM EMPLOYEE SUB 
WHERE SUB.DEPT_CODE = MAIN.DEPT_CODE/* (1) */)
ORDER BY HIRE_DATE;

-- 1) MAIN의 1행의 DEPT_CODE를 서브쿼리에 대입
-- 2) SUB를 수행
...
SELECT EMP_NAME, JOB_CODE, SALARY,
	(SELECT MAX(SALARY) FROM EMPLOYEE E) - SALARY
FROM EMPLOYEE E 

-- 각 직원들이 속한 직급의 급여 평균 조회
SELECT EMP_NAME, JOB_CODE, SALARY,
	(SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE E2 WHERE E2.JOB_CODE = E1.JOB_CODE) 평균
FROM EMPLOYEE E1
...
SELECT DEPT_CODE, DEPT_TITLE, CEIL(AVG(SALARY))
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON (DEPT_CODE = DEPT_ID)
...
-- 이름, 급여 순서로 조회된 결과를 TOP_SAL 로 지정
WITH TOP_SAL AS(SELECT EMP_NAME, SALARY FROM EMPLOYEE E ORDER BY SALARY DESC)
SELECT ROWNUM, EMP_NAME,SALARY
FROM TOP_SAL
WHERE ROWNUM <= 10;
