...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE E 
RIGHT /*OUTER*/ JOIN DEPARTMENT D ON (DEPT_CODE = DEPT_ID);
...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE E
FULL JOIN DEPARTMENT D ON (DEPT_CODE = DEPT_ID);
...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE E 
CROSS JOIN DEPARTMENT D; -- 207행 == 23 * 9 == 207
...
WHERE DEPT_CODE = DEPT_ID -- EMPLOUEE + DEPARTMENT 조인
AND LOCATION_ID = LOCAL_CODE; -- (EMPLOUEE + DEPARTMENT) + LOCATION 조인
...
JOIN DEPARTMENT D ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION L ON (LOCATION_ID = LOCAL_CODE)
WHERE JOB_NAME = '대리' AND LOCAL_NAME = 'ASIA1';
...
AND JOB_NAME = '대리' AND LOCAL_NAME = 'ASIA1';
...
SELECT EMP_NAME, JOB_NAME, SALARY, (SALARY * 12 * (1 + NVL(BONUS, 0))) 
FROM EMPLOYEE E
JOIN JOB J USING (JOB_CODE)
